...
\caption{\label{tbl:methods}
  Numerous exemplar methods within the framework. We coded
  methods into each design activity: \emph{understand}
  (\textbf{\emph{u}}), \emph{ideate} (\textbf{\emph{i}}), \emph{make}
  (\textbf{\emph{m}}), and \emph{deploy} (\textbf{\emph{d}}). We tagged
  generative (\textbf{\emph{g}}) and evaluative (\textbf{\emph{e}}) methods,
  along with those commonly reported within the visualization community
  (\textbf{\emph{v}}). We present definitions to
  assist designers employing methods. Appendix~\ref{sec:app-methods}
  contains a more extensive list of 100 methods. \\
}
\resizebox{\textwidth}{!}{
